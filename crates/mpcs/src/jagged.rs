...
mod tests {
    ...
    /// Brute-force compute h_j(x) for round j given challenges so far.
    /// This sums over all Boolean assignments to variables j..n-1 (0-indexed),
    /// with variables 0..j-1 bound to challenges, and variable j set to x.
    fn brute_force_round_poly(
        q_evals: &[F],
        cumulative_heights: &[usize],
        z_row: &[E],
        z_col: &[E],
        num_giga_vars: usize,
        challenges: &[E], // r_1..r_{j-1} (j-1 elements for round j, 1-indexed)
    ) -> [E; 3] {
        let eq_row = build_eq_x_r_vec(z_row);
        let eq_col = build_eq_x_r_vec(z_col);
        let total_evals = *cumulative_heights.last().unwrap();
        let n = num_giga_vars;
        let j = challenges.len(); // number of already-bound variables (0-indexed)
        let remaining = n - j - 1; // free variables after the current one

        let mut h = [E::ZERO; 3];

        // Iterate over all Boolean assignments to remaining variables
        for tail in 0..1usize << remaining {
            for x_bit in 0..2usize {
                // Build full index: challenges || x_bit || tail
                // In little-endian: bit 0 = variable 0 = challenges[0]'s bit, etc.
                // But challenges are extension field elements, not bits.
                // We need to compute the MLE evaluation at (challenges[0..j], x, tail_bits).
                // Instead, iterate over all Boolean prefixes and weight by eq.

                // Full index decomposition: a (j bits) || x_bit (1 bit) || tail (remaining bits)
                let mut q_val = E::ZERO;
                let mut f_val = E::ZERO;
                let eq_challenges = if j > 0 {
                    build_eq_x_r_vec(challenges)
                } else {
                    vec![E::ONE]
                };

                for a in 0..1usize << j {
                    let giga_idx = a | (x_bit << j) | (tail << (j + 1));
                    let weight = eq_challenges[a];

                    if giga_idx < total_evals {
                        let col_idx = cumulative_heights.partition_point(|&t| t <= giga_idx) - 1;
                        let row_idx = giga_idx - cumulative_heights[col_idx];
                        let qv: E = q_evals[giga_idx].into();
                        let fv = eq_row[row_idx] * eq_col[col_idx];
                        q_val += weight * qv;
                        f_val += weight * fv;
                    }
                }
        ...
        // h(2) via bilinear interpolation
        // For each tail, compute q(2) = 2*q(1) - q(0), f(2) = 2*f(1) - f(0)
        // But we need to recompute properly.
        // Actually, h(2) = sum_tail [q_bound(2, tail) * f_bound(2, tail)]
        // where q_bound(x, tail) = (1-x)*q_bound(0, tail) + x*q_bound(1, tail)
        // = 2*q_bound(1, tail) - q_bound(0, tail) for x=2
        // Similarly for f.
        // But h(2) = sum_tail (2q1 - q0)(2f1 - f0) != 2*h(1) - h(0)
        // because it's a product. So we need per-tail computation.
        h[2] = E::ZERO;
        for tail in 0..1usize << remaining {
            let eq_challenges = if j > 0 {
                build_eq_x_r_vec(challenges)
    ...
    fn test_m_table_correctness() {
        ...
        let z_col: Vec<E> = (0..1).map(|_| E::random(&mut rng)).collect();
        ...
        // Test epoch j'=1 (round 1, no prior challenges)
        let m1 = build_m_table(&input, &eq_row, &eq_col, &[], 1);
        let [h0, h1, h2] = compute_round_from_m(&m1, 1, &[]);
        let [ref_h0, ref_h1, ref_h2] = brute_force_round_poly(
            &q_evals,
            &cumulative_heights,
            &z_row,
            &z_col,
        ...
        assert_eq!(h0, ref_h0, "h(0) mismatch at round 1");
        assert_eq!(h1, ref_h1, "h(1) mismatch at round 1");
        assert_eq!(h2, ref_h2, "h(2) mismatch at round 1");

        // Test epoch j'=2 (rounds 2,3 with 1 prior challenge)
        let r1 = E::random(&mut rng);
        let m2 = build_m_table(&input, &eq_row, &eq_col, &[r1], 2);

        // Round 2: d=0 intra-epoch challenges
        let [h0, h1, h2] = compute_round_from_m(&m2, 2, &[]);
        let [ref_h0, ref_h1, ref_h2] = brute_force_round_poly(
            &q_evals,
            &cumulative_heights,
            &z_row,
        ...
        assert_eq!(h0, ref_h0, "h(0) mismatch at round 2");
        assert_eq!(h1, ref_h1, "h(1) mismatch at round 2");
        assert_eq!(h2, ref_h2, "h(2) mismatch at round 2");

        // Round 3: d=1 intra-epoch challenge
        let r2 = E::random(&mut rng);
        let [h0, h1, h2] = compute_round_from_m(&m2, 2, &[r2]);
        let [ref_h0, ref_h1, ref_h2] = brute_force_round_poly(
            &q_evals,
            &cumulative_heights,
            &z_row,
        ...
        assert_eq!(h0, ref_h0, "h(0) mismatch at round 3");
        assert_eq!(h1, ref_h1, "h(1) mismatch at round 3");
        assert_eq!(h2, ref_h2, "h(2) mismatch at round 3");
    }
